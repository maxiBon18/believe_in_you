...
- `test/` mirrors `lib/`: `lib/entry/domain/services/slot_status_service.dart` is tested by
  `test/entry/domain/services/slot_status_service_test.dart`.
...
- The instant a window closes, and the instant before it — a save at `windowEnd - 1ms` succeeds, a
  save at `windowEnd` does not
...
- A slot whose window closed before `installedAt`, asserting `NotApplicable` rather than `Skipped`
